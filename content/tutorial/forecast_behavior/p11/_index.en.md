...
Using the created prediction model, we predict the probability of the vacation acquisition achievement for this year's employee data.
In this tutorial, you will use sample data that has been prepared for prediction.
Unlike data for creating a prediction model (training), you do not use variables that indicate whether employees have met the vacation criteria.
...
Specify the prediction data `2_休暇取得（予測用）.csv` here.<br/>
{{% desktop_only %}}
...
Predict "Enough Number of Holidays (target)" displayed as "?"  from information such as "Job Type", and "Number of vacations taken in the last 3 months".
...
{{% desktop_only %}}
Please specify "Employee ID" in [Add the following variables to the first column].

Click [Predict and Save]. Make a prediction for each row and save the results.
...
For each employee, a prediction probability of vacation acquisition achievement is calculated.
