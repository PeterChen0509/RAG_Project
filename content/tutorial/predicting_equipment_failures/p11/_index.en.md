...
The failure probability is predicted for each piece of equipment (Operating equipment before inspection) using the created prediction model.
...
Specify the prediction data `2_機器の故障（予測用）.csv` here.<br/>
{{% desktop_only %}}
...
Predict "State (target)" displayed as "?"  from information such as "Equipment Type", "Maintenance Team". 
...
{{% desktop_only %}}
Please specify "Equipment ID" in [Add the following variables to the first column].
...
For each piece of equipment, a prediction probability of failure to purchase is calculated.
