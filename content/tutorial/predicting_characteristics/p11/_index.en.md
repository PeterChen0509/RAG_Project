...
Use the prediction model that you have created to predict the characteristics for a raw material mix that you have not yet tested.
In this tutorial, you will use sample data that has been prepared for prediction.
We have prepared 51 patterns of mixed quantity, which is the best mixed quantity with little changes.
...
Specify the prediction data `2_製品特性（予測用）.csv` here.<br/>
{{% desktop_only %}}
...
Predict "Characteristic Value (target)" displayed as "?"  from information such as "Fragrance material 1", and "Fragrance material 2".
...
{{% desktop_only %}}
Please specify "Experiment ID" in [Add the following variables to the first column].
...
For each mixing quantity, predicted characteristic values are calculated.
