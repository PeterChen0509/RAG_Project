...
この画面にて、予測用データを指定します。
今回のチュートリアルで使用する予測用データの内容を確認しましょう。
{{% /div_slide %}}

{{% div_slide "ステップ2" normal %}}
![](../img/t_slide16.png)

作成した予測モデルを利用して、まだ苦情タイプをラベリングしていないレビュー文に対して、苦情タイプを予測します。
...
予測用データである`2_顧客の声（予測用）.csv` を指定してください。<br/>
{{% desktop_only %}}
...
予測用データのプレビューが表示されます。
「投稿内容」「評価」などの情報から「？」と表示されている「苦情タイプ(予測対象)」を予測します。
{{% desktop_only %}}
...
予測結果は以下のような形式で出力されます（オプション設定によっては違う形式になります）。各顧客の声がどの苦情タイプに該当するかの予測確率が算出されています。
