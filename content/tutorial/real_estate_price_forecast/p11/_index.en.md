...
Using the created prediction model, predict the closing price of the property.
In this tutorial, you will use sample data that has been prepared for prediction.
Information about the property you want to predict is written as follows.
...
Specify the prediction data `2_成約価格（予測用）.csv` here.<br/>
{{% desktop_only %}}
...
Predict "Closing Price (prediction target)" displayed as "?"  from information such as "Junction Width", "Building Area".
...
{{% desktop_only %}}
Please specify "ID" in [Add the following variables to the first column].
...
The predicted closing price is calculated for each property.
