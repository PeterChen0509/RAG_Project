...
作成した予測モデルを利用して、各顧客（成約に至っていない顧客）に対して、成約確率を予測します。
...
予測用データである`2_サービス成約（予測用）.csv`を指定してください。<br/>
{{% desktop_only %}}
...
予測用データのプレビューが表示されます。
「業種」「従業員数」などの情報から、「？」と表示されている「成約有無（予測対象）」を予測します。
